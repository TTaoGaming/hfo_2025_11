...
class MissionIntent(BaseModel):
    ...
    disruptor_max: int = Field(default=3, ge=0, description="Maximum number of disruptors")
    evolution_strategy: str = Field(default="MAP-Elites+DSPy", description="Strategy for the Mutate phase")

    # Model Strategy (FinOps)
    coordination_model_group: str = Field(default="Cheap Navigators", description="Model group for coordination/planning")
    execution_model_group: str = Field(default="Cheap QD Swarm", description="Model group for execution/workers")
    excluded_models: List[str] = Field(default_factory=list, description="List of explicitly excluded models")
