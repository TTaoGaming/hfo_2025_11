...
class StigmergySignal(BaseModel):
    ...
    type: str = Field(..., description="Type of signal: 'discovery', 'alert', 'heartbeat'")
...
async def run_round(model: str, concurrency: int):
    ...
    logger.info(f"   Success Rate: {success_count}/{concurrency} ({success_count/concurrency*100:.1f}%)")
    logger.info(f"   Total Time: {total_time:.2f}s")
    logger.info(f"   Avg Latency: {avg_latency:.2f}s")
    logger.info(f"   RAM Impact: {mem_before}% -> {mem_after}% (Delta: {mem_after - mem_before}%)")
...
async def main():
    ...
    for score in scoreboard:
        logger.info(f"{score['model']:<20} | {score['success_rate']*100:.0f}%       | {score['avg_latency']:.2f}s      | {score['ram_peak']}%")
