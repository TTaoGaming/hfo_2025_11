...
class GitOpsAgent:
    ...
    def get_heartbeat_uptime(self) -> float:
        """Get heartbeat uptime percentage via NATS Proof-of-Swarm."""
        try:
            # Use the NATS-based guard for unfakeable proof
            output = self.run_command("python3 venom/guard_swarm_proof.py", check=False)
    ...
    async def execute_cycle(self):
        ...
        # Get Uptime & Determine Level
        uptime = self.get_heartbeat_uptime()
        if uptime >= 80.0:
            level_str = "HFO Level 1 (Active)"
        else:
            level_str = "HFO Level 0 (Incubating)"

        uptime_str = f"💓 Heartbeat Online (24h): {uptime:.2f}%"
        status_footer = f"Status: {level_str}\n{uptime_str}"
