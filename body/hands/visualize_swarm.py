...
async def main(stdscr):
    ...
    async def handler(msg):
        try:
            ...
            logs.append(f"[{datetime.now().strftime('%H:%M:%S')}] {agent_id}: {data.get('phase')}")
            if len(logs) > 10:
                logs.pop(0)
        except:
            pass
    ...
    while True:
        ...
        # Agent Table
        row = 3
        stdscr.addstr(row, 0, f"{'AGENT ID':<15} {'PHASE':<10} {'QUORUM':<15} {'LAST SEEN':<10}")
        row += 1
        stdscr.addstr(row, 0, "-" * 60)
        row += 1
        
        sorted_agents = sorted(agents.keys(), key=lambda x: int(x.split('_')[1]) if '_' in x else 0)
        ...
        for agent_id in sorted_agents:
            ...
            if phase == "Execute": attr = curses.A_BOLD
            if quorum == "QUORUM_REACHED": attr = attr | curses.A_UNDERLINE
            
            stdscr.addstr(row, 0, f"{agent_id:<15} {phase:<10} {quorum:<15} {info['timestamp']:<10}", attr)
            row += 1
