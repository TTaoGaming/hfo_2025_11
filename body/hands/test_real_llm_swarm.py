...
class AgentResponse(BaseModel):
    ...
    analysis: str = Field(..., description="The direct answer or analysis of the mission.")
    confidence: float = Field(..., description="Self-assessed confidence score (0.0-1.0).")
...
async def swarmlord_publisher(js, mission_text: str) -> UUID4:
    intent = MissionIntent(
        ...
        rationale="Testing Real LLM Integration",
        swarm_size=1
    )
    signal = MissionSignal(
        producer_id="Swarmlord-Real",
        mission=intent,
        ...
        while True:
            try:
                ...
                for msg in msgs:
                    ...
                    logger.info(f"   ⚡ {agent_id}: Processing: '{signal.mission.description}'")
                    ...
                    try:
                        response = client.chat.completions.create(
                            ...
                            messages=[
                                {"role": "system", "content": f"You are Agent {agent_id}, a {role.value} in Hive Fleet Obsidian. Be concise."},
                        ...
                        result = ResultSignal(
                            ...
                            metadata={"reasoning": response.reasoning, "model": MODEL_ID}
                        )
...
async def assimilator_collector(js, target_mission_id: UUID4):
    logger.info(f"🧠 Assimilator: Listening for results for Mission {target_mission_id}...")
    ...
    while (datetime.now() - start_time).seconds < 15:
        try:
            ...
            if signal.mission_id == target_mission_id:
                ...
                logger.info(f"   🗑️ Assimilator: Ignoring result for old mission {signal.mission_id}")
...
async def main():
    ...
    try:
        ...
        worker = asyncio.create_task(agent_worker(nc, "Gemini-Worker", AgentRole.SHAPER))
        
        # 2. Publish a real question
        await asyncio.sleep(1)
        mission_id = await swarmlord_publisher(js, "Explain the concept of 'Stigmergy' in 10 words or less.")
