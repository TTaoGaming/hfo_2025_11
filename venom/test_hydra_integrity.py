...
def test_hydra_parallelism_and_intelligence():
    ...
    # Assertions
    assert len(mission_artifacts) >= 3, "Should have at least 3 artifacts (Workers + Synthesis)"
    
    # Check for "Real Intelligence" (Keywords in body)
    bodies = [a["body"].lower() for a in mission_artifacts]
    combined_body = " ".join(bodies)
    
    assert "rust" in combined_body or "neon" in combined_body or "silence" in combined_body, \
        "Artifacts do not contain requested keywords. Is the LLM working?"
        
    # Check Metadata
    for a in mission_artifacts:
        assert a["meta"]["model"] == "x-ai/grok-4.1-fast", f"Wrong model used in {a['file']}"
