...
hexagon:
  ontos:
    id: 70110319-d9fc-43f8-9ff9-d9f176715673
    type: py
    owner: Swarmlord
  chronos:
    status: active
    urgency: 0.5
    decay: 0.5
    created: '2025-11-23T10:21:31.318722+00:00'
  topos:
    address: venom/steps/test_swarm_steps.py
    links: []
  telos:
    viral_factor: 0.0
    meme: test_swarm_steps.py
"""
...
def mission_intent(swarm_context, description):
    swarm_context["intent"] = MissionIntent(
        description=description, rationale="Test Rationale"
    )
...
@when(
    parsers.parse(
        'spawns {count:d} agents (including {disruptor_range} "Disruptors") to execute the "{loop}"'
    )
)
...
@when(
    parsers.parse(
        'the system "Mutates" the "{framework}" prompts and swarm parameters using "{method}"'
    )
)
def mutate_system(swarm_context, framework, method):
    ...
    swarm_context["swarm_state"].mutation_history.append(
        f"Applied {method} via {framework}"
    )
