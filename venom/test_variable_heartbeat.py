...
spec = importlib.util.spec_from_file_location(
    "cleanroom_prey_1111", "buds/hfo_gem_gen_55/body/hands/cleanroom_prey_1111.py"
)
...
class TestAgent(PreyAgent):  # type: ignore
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.forced_state = "Clear"

    async def perceive(self, loop_id: int) -> PerceptionReport:  # type: ignore
    ...
    async def execute(self, plan):
        ...
        return ExecutionResult(
            loop_id=plan.loop_id, status="success", output="Test", tool_outputs=[]
        )
...
async def test_variable_heartbeat():
    agent = TestAgent(
        agent_id="test_variable", stigmergy=MockStigmergy(), mission="Test"
    )
    ...
    for i in range(1, 4):
        ...
        print(
            f"Loop {i}: Elapsed {elapsed:.2f}s. State: {cynefin}. Sleeping {sleep_time:.2f}s..."
        )
    ...
    print(
        f"Loop 4: Elapsed {elapsed:.2f}s. State: {cynefin}. Sleeping {sleep_time:.2f}s..."
    )
    print(
        f"-> Calculated Sleep: {sleep_time:.2f}s. (If we waited, total would be ~60s)"
    )
