...
async def test_heartbeat_integrity():
    ...
    agent = PreyAgent(agent_id="venom_test_subject", stigmergy=MockStigmergy(), mission="Heartbeat Test")
    ...
    for i, signal in enumerate(buffer):
        ...
        if i > 0:
            ...
            if signal.previous_signal_id != prev_signal.id:
                logger.error(f"❌ Chain Break at {i}! PrevID {signal.previous_signal_id} != {prev_signal.id}")
                chain_broken = True
            else:
                logger.info(f"🔗 Link {i-1}->{i} Verified. Delta: {signal.delta_seconds:.4f}s")
        else:
