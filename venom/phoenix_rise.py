...
class PhoenixCouncil:
    ...
    async def debate(self, domain: str, files: Dict[str, str]) -> DomainStrategy:
        ...
        """
        logger.info(f"🔥 The Council is debating: {domain} ({len(files)} files)...")
        ...
        try:
            ...
            logger.error(f"❌ Council failed to debate {domain}: {e}")
...
async def main():
    ...
    if not library_path.exists():
        logger.error("❌ Library not found. Run ingestion first.")
        return
    ...
    with open(report_path, "w") as f:
        f.write("# 🔥 Phoenix Rise Report: Gen 51 Strategy\n\n")
    ...
    for domain, files in domains.items():
        ...
        if strategy:
            ...
            with open(report_path, "a") as f:
                f.write(f"## 🦅 Domain: {strategy.domain.upper()}\n\n")
                ...
                f.write("### ⚔️ Conflicts & Resolutions\n")
                ...
                f.write("### 📂 Gen 51 File Plan\n")
                for plan in strategy.gen51_files:
                    f.write(f"*   `{plan.filename}`: {plan.purpose}\n")
                f.write("\n---\n\n")

            logger.info(f"✅ {domain} strategy crystallized.")

    logger.info(f"✨ Phoenix Rise Report generated at {report_path}")
