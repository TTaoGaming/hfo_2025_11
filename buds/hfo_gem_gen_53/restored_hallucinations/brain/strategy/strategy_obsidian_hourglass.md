...
hexagon:
  ...
  chronos:
    ...
    generation: 51
  topos:
    address: brain/strategy_obsidian_hourglass.md
    links: []
  telos:
    viral_factor: 0.0
...
stateDiagram-v2
    ...
    state "Standard Flow (Gravity)" as Gravity {
        [*] --> Query_Karma
        Query_Karma --> Execute_Neck
        Execute_Neck --> Record_History
    }

    state "The Flip (Anti-Gravity)" as Flip {
        [*] --> Freeze_Neck
        Freeze_Neck --> Simulate_Future
        Simulate_Future --> Collapse_Wavefunction
        Collapse_Wavefunction --> Invert_Causality
        Invert_Causality --> Execute_Golden_Path
    }

    Gravity --> Flip : High Uncertainty Detected
