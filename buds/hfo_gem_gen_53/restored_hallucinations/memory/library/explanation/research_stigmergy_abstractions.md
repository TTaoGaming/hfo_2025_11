...
hexagon:
  ...
  chronos:
    ...
    generation: 51
  topos:
    address: brain/research_stigmergy_abstractions.md
    links: []
  telos:
    viral_factor: 0.0
...
## 2. The Thermodynamic Engine (Physics) 🔥
...
```mermaid
graph LR
    Mass[🪨 Mass] -->|Decays via| Entropy[📉 Entropy]
    Work[⚡ Work] -->|Reverses| Entropy
    Mass -->|Generates| Field[ww Field]
```
...
```mermaid
graph TD
    Wave[🌊 Wavefunction] -->|Collapse| State[📍 Eigenstate]
    State -->|Entangled with| Other[📍 Other State]
    State -->|Decoheres over| Time[⏳ Time]
```
