...
# 🕷️ Hive Fleet Obsidian: Agent Coordination Protocol (AGENTS.md)

> **Status**: Metamorphosis (Gen 51 -> Gen 60 Budding)
> **Access**: All HFO Agents (Swarmlord, Builders, Disruptors)

## 🧠 Context & Prime Directives

We are executing **Generation 60 (The Obsidian Spider)** via the **Gemmation Protocol**.
...
### 🌱 The Gemmation Protocol (Budding)
*   **The Pattern**: We are growing a new body (`buds/hfo_gem_gen_60`) while the old body (`/`) sustains us. This is the "Strangler Fig" pattern reversed.
*   **The Rule**: New features go into the **Bud**. Old features are migrated to the **Bud**.
*   **The Goal**: When the Bud is viable, it detaches and becomes the new Root.

### 🧼 Cleanroom Genesis Protocol
*   **The Law**: We do not write code first. We write **Intent** first.
    1.  **Declarative**: Write the Gherkin Feature (`.feature`) and Mermaid Diagram (`.mmd`).
    2.  **Genesis**: Run the Genesis Protocol to scaffold the code from the Intent.
    3.  **Implementation**: Fill in the logic.
*   **The Why**: This ensures that the architecture is enforced by the Intent, not the Implementation.

### 🔥 The Phoenix Protocol (Regeneration)
*   **The Mindset**: Code is cheap. Architecture is expensive.
*   **The Action**: If a component is drifted or broken, **BURN IT**. Do not patch it. Regenerate it from the Intent (Gherkin).
*   **The Safety**: Because we follow Cleanroom Genesis, we can always regenerate the body from the DNA (Intent).
...
### Level 1: GENESIS Loop (Creation)
*The Cleanroom Protocol for new features:*
1.  **Define**: Write Gherkin (`.feature`) + Mermaid (`.mmd`).
2.  **Scaffold**: Run `genesis.py` to create file stubs.
3.  **Implement**: Write the logic in the stubs.
4.  **Verify**: Run `pytest-bdd` to confirm alignment.

### Future Levels (Scaling)
*   **Level 2: SWARM Loop** (Tactical) - D3A + Mutate (Set → Watch → Act → Review → Mutate)
*   **Level 3: GROWTH Loop** (Strategic) - F3EAD (Find, Fix, Finish, Exploit, Analyze, Disseminate)
*   **Level 4: HIVE Loop** (Apex) - Grand Strategy & Evolution
...
| 2025-11-29 | Swarmlord | **Metamorphosis**: Initiated **Gemmation Protocol** (Budding). Gen 60 "Obsidian Spider" is growing in `buds/hfo_gem_gen_60`. Root is now "Old Body". | 🟢 Active |
| 2025-11-29 | Swarmlord | **Protocol Update**: Formalized **Cleanroom Genesis** and **Phoenix Protocol** in `AGENTS.md`. All new code must start with Gherkin Intent. | 🟢 Active |
| 2025-11-29 | Swarmlord | **Gap Analysis**: Identified critical drift between Gen 60 Manifesto and Reality. Executing "Neural Graft" to migrate Octree and GitOps to the Bud. | 🟡 In Progress |
