LLM Leaderboard - Comparison of over 100 AI models from OpenAI, Google, DeepSeek & others
Follow us on Twitter or LinkedIn to stay up to date with future analysis
Artificial Analysis
Artificial Analysis
For Enterprise
Insights


LLM Leaderboard - Comparison of over 100 AI models from OpenAI, Google, DeepSeek & others
Comparison and ranking the performance of over 100 AI models (LLMs) across key metrics including intelligence, price, performance and speed (output speed - tokens per second & latency - TTFT), context window & others. For more details including relating to our methodology, see our FAQs.

For comparison of API Providers hosting the models see

LLM API Providers Leaderboard
HIGHLIGHTS

Intelligence:
Gemini 3 Pro Preview logo Gemini 3 Pro Preview and GPT-5.1 (high) logo GPT-5.1 (high) are the highest intelligence models, followed by GPT-5 Codex (high) logo GPT-5 Codex (high) & GPT-5 (high) logo GPT-5 (high).
Output Speed (tokens/s):
Granite 3.3 8B logo Granite 3.3 8B (425 t/s) and Gemini 2.5 Flash-Lite (Sep) logo Gemini 2.5 Flash-Lite (Sep) (408 t/s) are the fastest models, followed by gpt-oss-120B (high) logo gpt-oss-120B (high) & gpt-oss-120B (low) logo gpt-oss-120B (low).
Latency (seconds):
DeepSeek-OCR logo DeepSeek-OCR (0.20s) and  Apriel-v1.5-15B-Thinker logo Apriel-v1.5-15B-Thinker (0.20s) are the lowest latency models, followed by Llama Nemotron Super 49B v1.5 logo Llama Nemotron Super 49B v1.5 & Command-R logo Command-R.
Price ($ per M tokens):
Gemma 3n E4B logo Gemma 3n E4B ($0.03) and Ministral 3B logo Ministral 3B ($0.04) are the cheapest models, followed by Gemma 2 9B logo Gemma 2 9B & DeepSeek-OCR logo DeepSeek-OCR.
Context Window:
Llama 4 Scout logo Llama 4 Scout (10m) and MiniMax-Text-01 logo MiniMax-Text-01 (4m) are the largest context window models, followed by Grok 4.1 Fast logo Grok 4.1 Fast & Grok 4 Fast logo Grok 4 Fast.
Prompt options
Filters
Frontier Models:
All

Frontier Models
Open Weights:
All

Open Source

Proprietary
Size Class:
All

Tiny

Small

Medium

Large
Reasoning:
All
Reasoning
Non Reasoning
Model Status:
Clear
All
Current Models
Filter, e.g. GPT, Meta
Expand Columns
Features

Intelligence

Price

Speed

Latency


Model

Context
Window

Creator

Artificial
Analysis
Intelligence Index

Blended
USD/1M Tokens

Median
Tokens/s

Latency
First Answer Chunk (s)
Further
Analysis
Gemini 3 Pro Preview
1m
Google logo
Google

73
$4.50
133
32.79
Model
Providers
GPT-5.1 (high)
400k
OpenAI logo
OpenAI

70
$3.44
126
19.46
Model
Providers
GPT-5 Codex (high)
400k
OpenAI logo
OpenAI

68
$3.44
123
22.41
Model
Providers
GPT-5 (high)
400k
OpenAI logo
OpenAI

68
$3.44
0
0.00
Model
Providers
Kimi K2 Thinking
256k
Moonshot AI logo
Moonshot AI

67
$1.07
61
0.67
Model
Providers
GPT-5 (medium)
400k
OpenAI logo
OpenAI

66
$3.44
0
0.00
Model
Providers
o3
200k
OpenAI logo
OpenAI

65
$3.50
162
14.84
Model
Providers
Grok 4
256k
xAI logo
xAI

65
$6.00
36
8.07
Model
Providers
GPT-5 mini (high)
400k
OpenAI logo
OpenAI

64
$0.69
0
0.00
Model
Providers
Grok 4.1 Fast
2m
xAI logo
xAI

64
$0.28
102
13.36
Model
Providers
Claude 4.5 Sonnet
1m
Anthropic logo
Anthropic

63
$6.00
71
2.10
Model
Providers
GPT-5 (low)
400k
OpenAI logo
OpenAI

62
$3.44
112
20.16
Model
Providers
MiniMax-M2
205k
MiniMax logo
MiniMax

61
$0.53
87
1.61
Model
Providers
GPT-5 mini (medium)
400k
OpenAI logo
OpenAI

61
$0.69
72
27.68
Model
Providers
gpt-oss-120B (high)
131k
OpenAI logo
OpenAI

61
$0.26
340
0.50
Model
Providers
Grok 4 Fast
2m
xAI logo
xAI

60
$0.28
200
3.69
Model
Providers
Gemini 2.5 Pro
1m
Google logo
Google

60
$3.44
41
15.52
Model
Providers
Claude 4.1 Opus
200k
Anthropic logo
Anthropic

59
$30.00
40
1.40
Model
Providers
DeepSeek V3.1 Terminus
128k
DeepSeek logo
DeepSeek

58
$0.80
0
0.00
Model
Providers
Qwen3 235B A22B 2507
256k
Alibaba logo
Alibaba

57
$2.63
76
1.34
Model
Providers
Grok 3 mini Reasoning (high)
1m
xAI logo
xAI

57
$0.35
141
0.57
Model
Providers
DeepSeek V3.2 Exp
128k
DeepSeek logo
DeepSeek

57
$0.32
30
1.15
Model
Providers
GLM-4.6
200k
Z AI logo
Z AI

56
$0.99
110
0.61
Model
Providers
Qwen3 Max Thinking
262k
Alibaba logo
Alibaba

56
$2.40
36
1.83
Model
Providers
Qwen3 Max
262k
Alibaba logo
Alibaba

55
$2.40
28
1.84
Model
Providers
Claude 4.5 Haiku
200k
Anthropic logo
Anthropic

55
$2.00
82
0.87
Model
Providers
Gemini 2.5 Flash (Sep)
1m
Google logo
Google

54
$0.85
150
8.32
Model
Providers
Qwen3 VL 235B A22B
262k
Alibaba logo
Alibaba

54
$2.63
43
1.37
Model
Providers
Qwen3 Next 80B A3B
262k
Alibaba logo
Alibaba

54
$1.88
170
1.14
Model
Providers
gpt-oss-20B (high)
131k
OpenAI logo
OpenAI

52
$0.09
257
0.51
Model
Providers
Magistral Medium 1.2
128k
Mistral logo
Mistral

52
$2.75
87
0.52
Model
Providers
DeepSeek R1 0528
128k
DeepSeek logo
DeepSeek

52
$2.36
0
0.00
Model
Providers
Qwen3 VL 32B
256k
Alibaba logo
Alibaba

52
$2.63
52
1.19
Model
Providers
Seed-OSS-36B-Instruct
512k
ByteDance Seed logo
ByteDance Seed

52
$0.30
30
1.81
Model
Providers
Apriel-v1.5-15B-Thinker
128k
ServiceNow logo
ServiceNow

52
$0.00
158
0.20
Model
Providers
GPT-5 nano (high)
400k
OpenAI logo
OpenAI

51
$0.14
0
0.00
Model
Providers
Kimi K2 0905
256k
Moonshot AI logo
Moonshot AI

50
$1.20
64
0.65
Model
Providers
Claude 4.5 Sonnet
1m
Anthropic logo
Anthropic

50
$6.00
72
2.09
Model
Providers
GPT-5 nano (medium)
400k
OpenAI logo
OpenAI

49
$0.14
0
0.00
Model
Providers
GLM-4.5-Air
128k
Z AI logo
Z AI

49
$0.42
102
0.64
Model
Providers
Grok Code Fast 1
256k
xAI logo
xAI

49
$0.53
253
10.05
Model
Providers
Gemini 2.5 Flash-Lite (Sep)
1m
Google logo
Google

48
$0.17
4
4.31
Model
Providers
gpt-oss-120B (low)
131k
OpenAI logo
OpenAI

48
$0.26
321
0.51
Model
Providers
Gemini 2.5 Flash (Sep)
1m
Google logo
Google

47
$0.85
223
0.30
Model
Providers
Qwen3 30B A3B 2507
262k
Alibaba logo
Alibaba

46
$0.75
176
1.20
Model
Providers
DeepSeek V3.2 Exp
128k
DeepSeek logo
DeepSeek

46
$0.32
27
1.13
Model
Providers
DeepSeek V3.1 Terminus
128k
DeepSeek logo
DeepSeek

46
$0.80
0
0.00
Model
Providers
Qwen3 235B 2507
256k
Alibaba logo
Alibaba

45
$1.23
40
1.50
Model
Providers
Qwen3 VL 30B A3B
256k
Alibaba logo
Alibaba

45
$0.75
107
1.04
Model
Providers
Llama Nemotron Super 49B v1.5
128k
NVIDIA logo
NVIDIA

45
$0.17
79
0.21
Model
Providers
Qwen3 Next 80B A3B
262k
Alibaba logo
Alibaba

45
$0.88
165
1.34
Model
Providers
Ling-1T
128k
InclusionAI logo
InclusionAI

45
$1.00
16
0.83
Model
Providers
GLM-4.6
200k
Z AI logo
Z AI

45
$0.99
37
0.72
Model
Providers
Claude 4.1 Opus
200k
Anthropic logo
Anthropic

45
$30.00
40
1.42
Model
Providers
gpt-oss-20B (low)
131k
OpenAI logo
OpenAI

44
$0.09
255
0.57
Model
Providers
Qwen3 VL 235B A22B
262k
Alibaba logo
Alibaba

44
$1.23
40
1.26
Model
Providers
GPT-5 (minimal)
400k
OpenAI logo
OpenAI

43
$3.44
72
0.81
Model
Providers
Qwen3 4B 2507
262k
Alibaba logo
Alibaba

43
$0.00
0
0.00
Model
Providers
Magistral Small 1.2
128k
Mistral logo
Mistral

43
$0.75
195
0.32
Model
Providers
GPT-5.1
400k
OpenAI logo
OpenAI

43
$3.44
82
0.82
Model
Providers
EXAONE 4.0 32B
131k
LG AI Research logo
LG AI Research

43
$0.70
93
0.41
Model
Providers
Qwen3 Coder 480B
262k
Alibaba logo
Alibaba

42
$3.00
41
1.78
Model
Providers
Ring-1T
128k
InclusionAI logo
InclusionAI

42
$1.00
14
0.96
Model
Providers
GPT-5 (ChatGPT)
128k
OpenAI logo
OpenAI

42
$3.44
113
0.67
Model
Providers
Claude 4.5 Haiku
200k
Anthropic logo
Anthropic

42
$2.00
105
0.95
Model
Providers
Gemini 2.5 Flash-Lite (Sep)
1m
Google logo
Google

42
$0.17
408
0.36
Model
Providers
GPT-5 mini (minimal)
400k
OpenAI logo
OpenAI

42
$0.69
70
0.78
Model
Providers
Hermes 4 405B
128k
Nous Research logo
Nous Research

42
$1.50
34
0.77
Model
Providers
Qwen3 VL 32B
256k
Alibaba logo
Alibaba

41
$1.23
46
1.16
Model
Providers
Qwen3 Omni 30B A3B
66k
Alibaba logo
Alibaba

40
$0.43
57
1.38
Model
Providers
Ring-flash-2.0
128k
InclusionAI logo
InclusionAI

40
$0.25
78
1.66
Model
Providers
Hermes 4 70B
128k
Nous Research logo
Nous Research

39
$0.20
87
0.61
Model
Providers
Grok 4 Fast
2m
xAI logo
xAI

39
$0.28
167
0.55
Model
Providers
Llama Nemotron Ultra
128k
NVIDIA logo
NVIDIA

38
$0.90
39
0.75
Model
Providers
Qwen3 VL 30B A3B
256k
Alibaba logo
Alibaba

38
$0.35
96
1.10
Model
Providers
Ling-flash-2.0
128k
InclusionAI logo
InclusionAI

38
$0.25
59
1.81
Model
Providers
Solar Pro 2
66k
Upstage logo
Upstage

38
$0.50
108
1.15
Model
Providers
NVIDIA Nemotron Nano 9B V2
131k
NVIDIA logo
NVIDIA

37
$0.07
95
0.23
Model
Providers
GLM-4.5V
64k
Z AI logo
Z AI

37
$0.85
57
1.49
Model
Providers
Qwen3 30B A3B 2507
262k
Alibaba logo
Alibaba

37
$0.35
114
1.09
Model
Providers
NVIDIA Nemotron Nano 9B V2
131k
NVIDIA logo
NVIDIA

36
$0.07
95
0.23
Model
Providers
Llama 4 Maverick
1m
Meta logo
Meta

36
$0.42
123
0.56
Model
Providers
Llama 3.3 Nemotron Super 49B
128k
NVIDIA logo
NVIDIA

35
$0.00
0
0.00
Model
Providers
Mistral Medium 3.1
128k
Mistral logo
Mistral

35
$0.80
40
0.45
Model
Providers
Qwen3 Coder 30B A3B
262k
Alibaba logo
Alibaba

33
$0.90
88
1.40
Model
Providers
ERNIE 4.5 300B A47B
131k
Baidu logo
Baidu

33
$0.48
23
1.04
Model
Providers
Hermes 4 405B
128k
Nous Research logo
Nous Research

33
$1.50
33
0.72
Model
Providers
Nova Premier
1m
Amazon logo
Amazon

32
$5.00
45
0.95
Model
Providers
Qwen3 VL 8B
256k
Alibaba logo
Alibaba

32
$0.66
63
1.28
Model
Providers
DeepSeek R1 0528 Qwen3 8B
33k
DeepSeek logo
DeepSeek

31
$0.07
63
0.88
Model
Providers
Qwen3 4B 2507
262k
Alibaba logo
Alibaba

30
$0.00
0
0.00
Model
Providers
EXAONE 4.0 32B
131k
LG AI Research logo
LG AI Research

30
$0.70
87
0.39
Model
Providers
Solar Pro 2
66k
Upstage logo
Upstage

30
$0.50
113
1.23
Model
Providers
Qwen3 Omni 30B A3B
66k
Alibaba logo
Alibaba

30
$0.43
34
1.59
Model
Providers
DeepSeek R1 Distill Llama 70B
128k
DeepSeek logo
DeepSeek

30
$0.88
93
0.96
Model
Providers
GPT-5 nano (minimal)
400k
OpenAI logo
OpenAI

29
$0.14
110
0.69
Model
Providers
Mistral Small 3.2
128k
Mistral logo
Mistral

29
$0.15
80
0.28
Model
Providers
MiniMax-Text-01
4m
MiniMax logo
MiniMax

28
$0.42
33
1.27
Model
Providers
Llama 4 Scout
10m
Meta logo
Meta

28
$0.24
108
0.60
Model
Providers
Llama 3.1 405B
128k
Meta logo
Meta

28
$4.19
24
0.86
Model
Providers
Llama 3.3 70B
128k
Meta logo
Meta

28
$0.62
98
0.54
Model
Providers
Devstral Medium
256k
Mistral logo
Mistral

28
$0.80
98
0.38
Model
Providers
Ling-mini-2.0
131k
InclusionAI logo
InclusionAI

28
$0.12
136
1.69
Model
Providers
Qwen3 VL 4B
256k
Alibaba logo
Alibaba

27
$0.00
0
0.00
Model
Providers
Devstral Small
256k
Mistral logo
Mistral

27
$0.15
131
0.35
Model
Providers
Qwen3 VL 8B
256k
Alibaba logo
Alibaba

27
$0.31
108
1.12
Model
Providers
Command A
256k
Cohere logo
Cohere

27
$4.38
58
0.32
Model
Providers
Exaone 4.0 1.2B
64k
LG AI Research logo
LG AI Research

27
$0.00
0
0.00
Model
Providers
Llama Nemotron Super 49B v1.5
128k
NVIDIA logo
NVIDIA

27
$0.17
70
0.24
Model
Providers
Llama 3.1 Nemotron Nano 4B v1.1
128k
NVIDIA logo
NVIDIA

26
$0.00
0
0.00
Model
Providers
GLM-4.5V
64k
Z AI logo
Z AI

26
$0.90
60
1.48
Model
Providers
Reka Flash 3
128k
Reka AI logo
Reka AI

26
$0.35
53
1.42
Model
Providers
Llama 3.3 Nemotron Super 49B
128k
NVIDIA logo
NVIDIA

26
$0.00
0
0.00
Model
Providers
Qwen3 VL 4B
256k
Alibaba logo
Alibaba

25
$0.00
0
0.00
Model
Providers
Nova Pro
300k
Amazon logo
Amazon

25
$1.40
0
0.00
Model
Providers
Hermes 4 70B
128k
Nous Research logo
Nous Research

24
$0.20
78
0.59
Model
Providers
Llama 3.1 Nemotron 70B
128k
NVIDIA logo
NVIDIA

24
$0.60
31
0.32
Model
Providers
Granite 4.0 H Small
128k
IBM logo
IBM

23
$0.11
226
8.85
Model
Providers
Phi-4
16k
Microsoft Azure logo
Microsoft Azure

23
$0.22
35
0.42
Model
Providers
Gemma 3 27B
128k
Google logo
Google

22
$0.00
49
0.65
Model
Providers
Nova Lite
300k
Amazon logo
Amazon

21
$0.10
158
0.40
Model
Providers
Jamba Reasoning 3B
262k
AI21 Labs logo
AI21 Labs

21
$0.00
0
0.00
Model
Providers
Jamba 1.7 Large
256k
AI21 Labs logo
AI21 Labs

21
$3.50
42
0.85
Model
Providers
Exaone 4.0 1.2B
64k
LG AI Research logo
LG AI Research

20
$0.00
0
0.00
Model
Providers
Gemma 3 12B
128k
Google logo
Google

20
$0.00
48
1.34
Model
Providers
Codestral (Jan)
256k
Mistral logo
Mistral

20
$0.45
167
0.31
Model
Providers
R1 1776
128k
Perplexity logo
Perplexity

19
$0.00
0
0.00
Model
Providers
Llama 3.2 90B (Vision)
128k
Meta logo
Meta

19
$0.72
36
0.35
Model
Providers
Nova Micro
130k
Amazon logo
Amazon

18
$0.06
276
0.34
Model
Providers
LFM2 8B A1B
33k
Liquid AI logo
Liquid AI

17
$0.00
0
0.00
Model
Providers
Granite 4.0 Micro
128k
IBM logo
IBM

16
$0.00
0
0.00
Model
Providers
Phi-4 Mini
128k
Microsoft Azure logo
Microsoft Azure

16
$0.00
46
0.31
Model
Providers
DeepHermes 3 - Mistral 24B
32k
Nous Research logo
Nous Research

16
$0.00
0
0.00
Model
Providers
Llama 3.2 11B (Vision)
128k
Meta logo
Meta

16
$0.16
54
0.40
Model
Providers
Gemma 3n E4B
32k
Google logo
Google

15
$0.03
52
0.36
Model
Providers
Jamba 1.7 Mini
258k
AI21 Labs logo
AI21 Labs

15
$0.25
154
0.70
Model
Providers
Gemma 3 4B
128k
Google logo
Google

15
$0.00
49
0.92
Model
Providers
OLMo 2 32B
4k
Allen Institute for AI logo
Allen Institute for AI

14
$0.24
18
0.73
Model
Providers
Granite 4.0 H 1B
128k
IBM logo
IBM

14
$0.00
0
0.00
Model
Providers
Aya Expanse 32B
128k
Cohere logo
Cohere

14
$0.75
44
0.30
Model
Providers
Granite 4.0 1B
128k
IBM logo
IBM

13
$0.00
0
0.00
Model
Providers
Phi-4 Multimodal
128k
Microsoft Azure logo
Microsoft Azure

12
$0.00
17
0.34
Model
Providers
Ministral 8B
128k
Mistral logo
Mistral

12
$0.10
160
0.30
Model
Providers
LFM2 2.6B
33k
Liquid AI logo
Liquid AI

12
$0.00
0
0.00
Model
Providers
Gemma 3n E2B
32k
Google logo
Google

11
$0.00
51
0.38
Model
Providers
Ministral 3B
128k
Mistral logo
Mistral

11
$0.04
237
0.28
Model
Providers
Aya Expanse 8B
8k
Cohere logo
Cohere

10
$0.75
83
0.27
Model
Providers
LFM2 1.2B
33k
Liquid AI logo
Liquid AI

10
$0.00
0
0.00
Model
Providers
OLMo 2 7B
4k
Allen Institute for AI logo
Allen Institute for AI

10
$0.00
0
0.00
Model
Providers
Molmo 7B-D
4k
Allen Institute for AI logo
Allen Institute for AI

9
$0.00
0
0.00
Model
Providers
Granite 4.0 H 350M
33k
IBM logo
IBM

8
$0.00
0
0.00
Model
Providers
Granite 4.0 350M
33k
IBM logo
IBM

8
$0.00
0
0.00
Model
Providers
Gemma 3 1B
32k
Google logo
Google

7
$0.00
45
0.53
Model
Providers
Gemma 3 270M
32k
Google logo
Google

6
$0.00
0
0.00
Model
Providers
DeepHermes 3 - Llama-3.1 8B
128k
Nous Research logo
Nous Research

2
$0.00
0
0.00
Model
Providers
DeepSeek-OCR
8k
DeepSeek logo
DeepSeek

--
$0.05
316
0.20
Model
Providers
Cogito v2.1
128k
Deep Cogito logo
Deep Cogito

--
$0.00
0
0.00
Model
Providers
Cogito v2.1
128k
Deep Cogito logo
Deep Cogito

--
$0.00
0
0.00
Model
Providers
Key definitions
Back to Navigation

Context window


Output Speed


Latency (Time to First Token)


Price


Output Price


Input Price


Time period

Models compared: OpenAI: GPT 4o Audio, GPT 4o Realtime, GPT 4o Speech Pipeline, GPT Realtime, GPT Realtime Mini (Oct '25), GPT-3.5 Turbo, GPT-3.5 Turbo (0125), GPT-3.5 Turbo (0301), GPT-3.5 Turbo (0613), GPT-3.5 Turbo (1106), GPT-3.5 Turbo Instruct, GPT-4, GPT-4 Turbo, GPT-4 Turbo (0125), GPT-4 Turbo (1106), GPT-4 Vision, GPT-4.1, GPT-4.1 mini, GPT-4.1 nano, GPT-4.5 (Preview), GPT-4o (Apr), GPT-4o (Aug), GPT-4o (ChatGPT), GPT-4o (Mar), GPT-4o (May), GPT-4o (Nov), GPT-4o Realtime (Dec), GPT-4o mini, GPT-4o mini Realtime (Dec), GPT-5 (ChatGPT), GPT-5 (high), GPT-5 (low), GPT-5 (medium), GPT-5 (minimal), GPT-5 Codex (high), GPT-5 Pro (high), GPT-5 mini (high), GPT-5 mini (medium), GPT-5 mini (minimal), GPT-5 nano (high), GPT-5 nano (medium), GPT-5 nano (minimal), GPT-5.1, GPT-5.1 (high), GPT-5.1 Codex (high), GPT-5.1 Codex mini (high), gpt-oss-120B (high), gpt-oss-120B (low), gpt-oss-20B (high), gpt-oss-20B (low), o1, o1-mini, o1-preview, o1-pro, o3, o3-mini, o3-mini (high), o3-pro, and o4-mini (high), Meta: Code Llama 70B, Llama 2 Chat 13B, Llama 2 Chat 70B, Llama 2 Chat 7B, Llama 3 70B, Llama 3 8B, Llama 3.1 405B, Llama 3.1 70B, Llama 3.1 8B, Llama 3.2 11B (Vision), Llama 3.2 1B, Llama 3.2 3B, Llama 3.2 90B (Vision), Llama 3.3 70B, Llama 4 Behemoth, Llama 4 Maverick, Llama 4 Scout, and Llama 65B, Google: Gemini 1.0 Pro, Gemini 1.0 Ultra, Gemini 1.5 Flash (May), Gemini 1.5 Flash (Sep), Gemini 1.5 Flash-8B, Gemini 1.5 Pro (May), Gemini 1.5 Pro (Sep), Gemini 2.0 Flash, Gemini 2.0 Flash (exp), Gemini 2.0 Flash Thinking exp. (Dec), Gemini 2.0 Flash Thinking exp. (Jan), Gemini 2.0 Flash-Lite (Feb), Gemini 2.0 Flash-Lite (Preview), Gemini 2.0 Pro Experimental, Gemini 2.5 Flash, Gemini 2.5 Flash Live Preview, Gemini 2.5 Flash Native Audio, Gemini 2.5 Flash Native Audio Dialog, Gemini 2.5 Flash (Sep), Gemini 2.5 Flash-Lite, Gemini 2.5 Flash-Lite (Sep), Gemini 2.5 Pro, Gemini 2.5 Pro (Mar), Gemini 2.5 Pro (May), Gemini 3 Pro Preview, Gemini 3 Pro Preview (low), Gemini Experimental (Nov), Gemma 2 27B, Gemma 2 2B, Gemma 2 9B, Gemma 3 12B, Gemma 3 1B, Gemma 3 270M, Gemma 3 27B, Gemma 3 4B, Gemma 3n E2B, Gemma 3n E4B, Gemma 3n E4B (May), Gemma 7B, PALM-2, and Whisperwind, Anthropic: Claude 2.0, Claude 2.1, Claude 3 Haiku, Claude 3 Opus, Claude 3 Sonnet, Claude 3.5 Haiku, Claude 3.5 Sonnet (June), Claude 3.5 Sonnet (Oct), Claude 3.7 Sonnet, Claude 4 Opus, Claude 4 Sonnet, Claude 4.1 Opus, Claude 4.5 Haiku, Claude 4.5 Sonnet, and Claude Instant, Mistral: Codestral (Jan), Codestral (May), Codestral-Mamba, Devstral Medium, Devstral Small, Devstral Small (May), Magistral Medium 1, Magistral Medium 1.1, Magistral Medium 1.2, Magistral Small 1, Magistral Small 1.1, Magistral Small 1.2, Ministral 3B, Ministral 8B, Mistral 7B, Mistral Large (Feb), Mistral Large 2 (Jul), Mistral Large 2 (Nov), Mistral Medium, Mistral Medium 3, Mistral Medium 3.1, Mistral NeMo, Mistral Saba, Mistral Small (Feb), Mistral Small (Sep), Mistral Small 3, Mistral Small 3.1, Mistral Small 3.2, Mixtral 8x22B, Mixtral 8x7B, Pixtral 12B, and Pixtral Large, DeepSeek: DeepSeek Coder V2 Lite, DeepSeek LLM 67B (V1), DeepSeek Prover V2 671B, DeepSeek R1 (FP4), DeepSeek R1 (Jan), DeepSeek R1 0528, DeepSeek R1 0528 Qwen3 8B, DeepSeek R1 Distill Llama 70B, DeepSeek R1 Distill Llama 8B, DeepSeek R1 Distill Qwen 1.5B, DeepSeek R1 Distill Qwen 14B, DeepSeek R1 Distill Qwen 32B, DeepSeek V3 (Dec), DeepSeek V3 0324, DeepSeek V3.1, DeepSeek V3.1 Terminus, DeepSeek V3.2 Exp, DeepSeek-Coder-V2, DeepSeek-OCR, DeepSeek-V2, DeepSeek-V2.5, DeepSeek-V2.5 (Dec), DeepSeek-VL2, and Janus Pro 7B, Perplexity: PPLX-70B Online, PPLX-7B-Online, R1 1776, Sonar, Sonar 3.1 Huge, Sonar 3.1 Large, Sonar 3.1 Small , Sonar Large, Sonar Pro, Sonar Reasoning, Sonar Reasoning Pro, and Sonar Small, xAI: Grok 2, Grok 3, Grok 3 Reasoning Beta, Grok 3 mini, Grok 3 mini Reasoning (low), Grok 3 mini Reasoning (high), Grok 4, Grok 4 Fast, Grok 4 Fast 1111 (Reasoning), Grok 4 mini (0908), Grok 4.1 Fast, Grok 4.1 Fast v4, Grok Beta, Grok Code Fast 1, Grok-1, and test model, OpenChat: OpenChat 3.5, Amazon: Nova 2.0 Lite, Nova 2.0 Lite (high), Nova 2.0 Lite (low), Nova 2.0 Lite (medium), Nova Lite, Nova Micro, Nova Premier, and Nova Pro, Microsoft Azure: Phi-3 Medium 14B, Phi-3 Mini, Phi-4, Phi-4 Mini, Phi-4 Multimodal, Phi-4 mini reasoning, Phi-4 reasoning, and Phi-4 reasoning plus, Liquid AI: LFM 1.3B, LFM 3B, LFM 40B, LFM2 1.2B, LFM2 2.6B, and LFM2 8B A1B, Upstage: Solar Mini, Solar Pro, Solar Pro (Nov), Solar Pro 2, and Solar Pro 2 , Databricks: DBRX, MiniMax: MiniMax M1 40k, MiniMax M1 80k, MiniMax-M2, and MiniMax-Text-01, NVIDIA: Cosmos Nemotron 34B, Llama 3.1 Nemotron 70B, Llama 3.1 Nemotron Nano 4B v1.1, Llama 3.1 Nemotron Nano 8B, Llama 3.3 Nemotron Nano 8B, Llama Nemotron Ultra, Llama 3.3 Nemotron Super 49B, Llama Nemotron Super 49B v1.5, NVIDIA Nemotron Nano 12B v2 VL, and NVIDIA Nemotron Nano 9B V2, StepFun: Step-2, Step-2-Mini, Step3, step-1-128k, step-1-256k, step-1-32k, step-1-8k, step-1-flash, step-2-16k-exp, and step-r1-v-mini, IBM: Granite 3.0 2B, Granite 3.0 8B, Granite 3.3 8B, Granite 4.0 1B, Granite 4.0 350M, Granite 4.0 8B, Granite 4.0 H 1B, Granite 4.0 H 350M, Granite 4.0 H Small, Granite 4.0 Micro, Granite 4.0 Tiny, and Granite Vision 3.3 2B, Inceptionlabs: Mercury, Mercury Coder Mini, Mercury Coder Small, and Mercury Instruct, Reka AI: Reka Core, Reka Edge, Reka Flash (Feb), Reka Flash, Reka Flash 3, and Reka Flash 3.1, LG AI Research: EXAONE 4.0 32B, EXAONE Deep 32B, and Exaone 4.0 1.2B, Xiaomi: MiMo 7B RL, Baidu: ERNIE 4.5, ERNIE 4.5 0.3B, ERNIE 4.5 21B A3B, ERNIE 4.5 300B A47B, ERNIE 4.5 VL 28B A3B, ERNIE 4.5 VL 424B A47B, and ERNIE X1, Baichuan: Baichuan 4 and Baichuan M1 (Preview), vercel: v0-1.0-md, Apple: Apple On-Device and FastVLM, Other: LLaVA-v1.5-7B, Tencent: Hunyuan A13B, Hunyuan 80B A13B, Hunyuan T1, and Hunyuan-TurboS, Z AI: GLM-4 32B, GLM-4 9B, GLM-4-Air, GLM-4 AirX, GLM-4 FlashX, GLM-4-Long, GLM-4-Plus, GLM-4.1V 9B Thinking, GLM-4.5, GLM-4.5-Air, GLM-4.5V, GLM-4.6, GLM-Z1 32B, GLM-Z1 9B, GLM-Z1 Rumination 32B, and GLM-Zero (Preview), Cohere: Aya Expanse 32B, Aya Expanse 8B, Command, Command A, Command Light, Command R7B, Command-R, Command-R (Mar), Command-R+ (Apr), and Command-R+, Bytedance: Duobao 1.5 Pro, Seed-Thinking-v1.5, Skylark Lite, and Skylark Pro, AI21 Labs: Jamba 1.5 Large, Jamba 1.5 Large (Feb), Jamba 1.5 Mini, Jamba 1.5 Mini (Feb), Jamba 1.6 Large, Jamba 1.6 Mini, Jamba 1.7 Large, Jamba 1.7 Mini, Jamba Instruct, and Jamba Reasoning 3B, Snowflake: Arctic and Snowflake Llama 3.3 70B, PaddlePaddle: PaddleOCR-VL-0.9B, Alibaba: QwQ-32B, QwQ 32B-Preview, Qwen Chat 14B, Qwen Chat 72B, Qwen Chat 7B, Qwen1.5 Chat 110B, Qwen1.5 Chat 14B, Qwen1.5 Chat 32B, Qwen1.5 Chat 72B, Qwen1.5 Chat 7B, Qwen2 72B, Qwen2 Instruct 7B, Qwen2 Instruct A14B 57B, Qwen2-VL 72B, Qwen2.5 Coder 32B, Qwen2.5 Coder 7B , Qwen2.5 Instruct 14B, Qwen2.5 Instruct 32B, Qwen2.5 72B, Qwen2.5 Instruct 7B, Qwen2.5 Max, Qwen2.5 Max 01-29, Qwen2.5 Omni 7B, Qwen2.5 Plus, Qwen2.5 Turbo, Qwen2.5 VL 72B, Qwen2.5 VL 7B, Qwen3 0.6B, Qwen3 1.7B, Qwen3 14B, Qwen3 235B, Qwen3 235B A22B 2507, Qwen3 235B 2507, Qwen3 30B, Qwen3 30B A3B 2507, Qwen3 32B, Qwen3 4B, Qwen3 4B 2507, Qwen3 8B, Qwen3 Coder 30B A3B, Qwen3 Coder 480B, Qwen3 Max, Qwen3 Max (Preview), Qwen3 Max Thinking, Qwen3 Next 80B A3B, Qwen3 Omni 30B A3B, Qwen3 VL 235B A22B, Qwen3 VL 30B A3B, Qwen3 VL 32B, Qwen3 VL 4B, and Qwen3 VL 8B, InclusionAI: Ling-1T, Ling-flash-2.0, Ling-mini-2.0, Ring-1T, and Ring-flash-2.0, 01.AI: Yi-Large and Yi-Lightning, and ByteDance Seed: Doubao Seed Code and Seed-OSS-36B-Instruct.

Footer
Key Links
Compare Language Models
Language Models Leaderboard
Language Model API Leaderboard
Image Arena
Video Arena
Speech Arena
Artificial Analysis
FAQ
Contact & Data access
Terms of Use
Privacy Policy
hello@artificialanalysis.ai
Subscribe to our newsletter
Email address
Newsletter
Subscribe
Twitter
LinkedIn
